...
module.exports = (function() {
  ...
  var handleCO2 = function(data) {
    ...
    this.emit("data", this._config.feedId, { timestamp: Date.now(), co2: parseInt(co2) });
  };

  var handleHumidity = function(data) {
    var humidity = data.substr(2);
    this.emit("data", this._config.feedId, { timestamp: Date.now(), humidity: parseInt(humidity)/10 });
  };

  var handleTemperature = function(data) {
    var temp = data.substr(2);
    this.emit("data", this._config.feedId, { timestamp: Date.now(), temperature: (parseInt(temp) - 1000)/10});
  };
